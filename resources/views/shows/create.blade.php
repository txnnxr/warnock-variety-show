...
@section('content')
    ...
    <form class="form-check"
        ...
        <textarea type="text" name="description" class="form-control col-9" rows="6">@if(isset($show))
                {{htmlspecialchars_decode($show->description)}}
            @endif</textarea>
        ...
        <input class="form-control col-9" type="number" name="max_attendants" id="maxAttendants" value=@if(isset($show))
            {{$show->max_attendants}}
                @endif >
        <label class="form-label col-3" for="maxTalents">Max Talents:</label>
        <input class="form-control col-9" type="number" name="max_talents" id="maxTalents" value=@if(isset($show))
            {{$show->max_talents}}
                @endif >
        <button class="form-control btn btn-primary" type="submit">@if(isset($show))
                Edit
            @else
                Create
            @endif Show
