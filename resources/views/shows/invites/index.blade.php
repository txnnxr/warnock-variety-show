            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{action([\App\Http\Controllers\ShowController::class, 'togglePublicRsvp'], ['show' => $show])}}" method="POST">
                        @csrf
                        <button class="btn btn-primary width col-12" type="submit">@if($show->public_rsvp_open)
                                Close
                            @else
                                Open
                            @endif</button>
                ...
                @foreach($show->invites as $invite)
                    <tr>
                        ...
                        <td>{{$invite->first_name}} {{$invite->middle_name}}
                            {{$invite->last_name}} @if($invite->has_plus_one_option)
                                (+1)
                            @endif</td>
                        {{--                        <td>@if($invite->phone){{$invite->phone}}@else{{$invite->email}}@endif</td>--}}
                        <td>{{$invite->response_status}} @if($invite->guest_request)
                                - REQUESTED
                            @endif</td>
                        <td>@if($invite->talent)
                                YES
                            ...
                            @endif</td>
                        <td>
                            <a href="{{action([\App\Http\Controllers\InvitationController::class, 'edit'], ['invite' => $invite])}}" class="btn btn-secondary col">Edit</a>
                            @if($invite->response_status == 'CREATED')
                                @if($invite->phone || $invite->email)
                                    <form class="d-inline-block" action="{{action([\App\Http\Controllers\InvitationController::class, 'send'], ['invite' => $invite])}}" method="POST">
                                        @csrf
